...
- PR base: `v4-series`
- state at this snapshot: open and draft; local branch has advanced beyond the
  remote PR head and still needs push/update
- local continuation state on 2026-07-17:
  - `origin/v4-series` has already been merged locally into the issue branch
  - the native reflow helper now lives only at
    `NativeUI/AddonHandlers/Quest/JournalDetailNativeBodyFlowHelper.cs`
  - the previous shared helper path is intentionally gone
  - remaining work is local validation, in-game acceptance, and PR refresh

The new worktree must start from the local branch, not only
`origin/issue-181-journaldetail-reflow`, or the merge + helper-localization
...
The branch work that mattered has already been narrowed locally:

- richer original text and geometry snapshots remain in `JournalDetailHandler`
- body-block ordering and section resolution now feed a
  `JournalDetail`-local helper
- internal container growth stays confined to the `JournalDetail` body flow
- the old shared-helper experiment has been retired in favor of the local file
  under `NativeUI/AddonHandlers/Quest/`

Do not reintroduce a shared reflow abstraction during this front. If another
surface later needs native reflow, open a new focused issue and branch for it.
...
Validation run date: 2026-07-18

Current local results:

- `dotnet restore .\Echoglossian.Tests\Echoglossian.Tests.csproj`: PASS
- `dotnet build .\Echoglossian.sln -c Debug --no-restore`: PASS
- `dotnet build .\Echoglossian.Tests\Echoglossian.Tests.csproj -c Debug --no-restore`: PASS
- `dotnet test .\Echoglossian.Tests\Echoglossian.Tests.csproj -c Debug --no-build`: PASS
  (`651` tests)
- `dotnet restore .\Echoglossian.Mock\Echoglossian.Mock.csproj`: PASS
- `dotnet build .\Echoglossian.Mock\Echoglossian.Mock.csproj -c Debug --no-restore`: PASS
- `dotnet restore .\Echoglossian.Mock.Tests\Echoglossian.Mock.Tests.csproj`: PASS
- `dotnet build .\Echoglossian.Mock.Tests\Echoglossian.Mock.Tests.csproj -c Debug --no-restore`: PASS
- `dotnet test .\Echoglossian.Mock.Tests\Echoglossian.Mock.Tests.csproj -c Debug --filter DalaMockHostCompatibilityGuardTests`: PASS
  (`3` tests)
- `dotnet test .\Echoglossian.Mock.Tests\Echoglossian.Mock.Tests.csproj -c Debug --no-build`: FAIL
- `.\Echoglossian.Mock\bin\x64\Debug\win-x64\Echoglossian.Mock.exe --check-compatibility`: INCOMPATIBLE
- `.\scripts\validate-local-tests.ps1`: PASS with `.Mock` startup rail skipped
  after a compatibility probe

The current `.Mock` failure occurs before the plugin startup assertions run:

- `DalaMock.Core` package version in both mock projects: `6.1.7`
- failing location: `Echoglossian.Mock.Tests/TestBoot.cs`, inside
  `new MockContainer(...)`
- failing exception family:
  `ReflectionTypeLoadException` / `TypeLoadException`
- failing missing member:
  `DalaMock.Core.Mocks.DalamudServices.MockFramework.CreateDebouncer`

Treat this as a DalaMock host/runtime drift blocker until proven otherwise. The
failure happens before `JournalDetail` reflow code is exercised, so it should
not be treated as evidence of a native reflow regression by itself.

Current local mitigation on this branch:

- `Echoglossian.Mock` now exposes `--check-compatibility`
- `scripts/validate-local-tests.ps1` runs that probe before the `.Mock` startup
  tests
- when the probe detects the known `IFramework.CreateDebouncer(...)` drift, the
  script skips `Echoglossian.Mock.Tests` instead of failing opaquely inside
  `new MockContainer(...)`
- direct `.Mock` startup attempts still fail fast with an explicit diagnostic so
  the drift remains visible for follow-up work
