...
public class MockSeStringEvaluator : ISeStringEvaluator, IMockService
{
    private readonly IDataManager dataManager;
    private readonly IClientState clientState;
    private readonly IGameConfig gameConfig;
    private readonly IPlayerState playerState;
    private readonly SheetRedirectResolver sheetRedirectResolver;
    private readonly NounProcessor nounProcessor;
    private readonly MockMacroDecoder macroDecoder;
    ...
    public MockSeStringEvaluator(IDataManager dataManager, IClientState clientState, IGameConfig gameConfig, IPlayerState playerState, SheetRedirectResolver sheetRedirectResolver, NounProcessor nounProcessor, MockMacroDecoder macroDecoder, MockDalamudConfiguration mockDalamudConfiguration)
    {
        this.dataManager = dataManager;
        this.clientState = clientState;
        this.gameConfig = gameConfig;
        this.playerState = playerState;
        this.sheetRedirectResolver = sheetRedirectResolver;
        this.nounProcessor = nounProcessor;
        this.macroDecoder = macroDecoder;
        this.dalamudConfiguration = mockDalamudConfiguration;
    }
        ...
        ClientLanguage? language = null)
    {
        ...
        if (!this.dataManager.GetExcelSheet<LSheets.Addon>(lang).TryGetRow(addonId, out var addonRow))
        {
            return default;
        }
    ...
    private LuminaSeStringBuilder EvaluateAndAppendTo(
        LuminaSeStringBuilder builder,
        ReadOnlySeStringSpan str,
    ...
    private unsafe bool TryResolveSetResetTime(in SeStringContext context, in ReadOnlySePayloadSpan payload)
    {
        ...
        this.macroDecoder.GetMacroTime()->SetTime(targetDate.ToLocalTime());
    ...
    private unsafe bool TryResolveSetTime(in SeStringContext context, in ReadOnlySePayloadSpan payload)
    {
        ...
        this.macroDecoder.GetMacroTime()->SetTime(date);
    ...
    private bool TryResolveSheet(in SeStringContext context, in ReadOnlySePayloadSpan payload)
    {
        ...
        var text = this.FormatSheetValue(
            context.Language,
            resolvedSheetName,
            eRowIdValue,
            eColIndexValue,
    ...
    private bool TryResolveSheetSub(in SeStringContext context, in ReadOnlySePayloadSpan payload)
    {
        ...
        var secondaryRowId = this.GetSubrowSheetIntValue(
            context.Language,
            eSheetNameStr.ExtractText(),
            eRowIdValue,
            (ushort)eSubrowIdValue,
            eColIndexValue);
        if (secondaryRowId == -1)
        {
            return false;
        ...
        var text = this.FormatSheetValue(
            context.Language,
            eSecondarySheetNameStr.ExtractText(),
            (uint)secondaryRowId,
            secondaryColIndex,
            0);
        if (text.IsEmpty)
    ...
    private int GetSubrowSheetIntValue(
        ClientLanguage language,
        string sheetName,
        uint rowId,
        ushort subrowId,
    ...
    private ReadOnlySeString FormatSheetValue(
        ClientLanguage language,
        string sheetName,
        uint rowId,
        uint colIndex,
        uint colParam)
    {
        ...
        return column.Type switch
        {
            ...
            ExcelColumnDataType.Bool => (row.ReadBool(column.Offset) ? 1u : 0).ToString(
                "D",
            ...
            ExcelColumnDataType.PackedBool1 => (row.ReadPackedBool(column.Offset, 1) ? 1u : 0).ToString(
                "D",
                CultureInfo.InvariantCulture),
            ExcelColumnDataType.PackedBool2 => (row.ReadPackedBool(column.Offset, 2) ? 1u : 0).ToString(
                "D",
                CultureInfo.InvariantCulture),
    ...
    private void AddSheetRedirectItemDecoration(
        in SeStringContext context,
        ref ReadOnlySeString text,
        SheetRedirectFlags flags,
        uint eRowIdValue)
    {
        ...
        if (!skipLink)
            sb.PushLink(
                LinkMacroPayloadType.Item,
                itemId,
                rarity,
                0u); // arg3 = some LogMessage flag based on LogKind RowId? => "89 5C 24 20 E8 ?? ?? ?? ?? 48 8B 1F"
        ...
        if (flags.HasFlag(SheetRedirectFlags.HighQuality)
            && this.dataManager.GetExcelSheet<LSheets.Addon>(context.Language).TryGetRow(9, out var hqSymbol))
        {
            sb.Append(hqSymbol.Text);
        }
        else if (flags.HasFlag(SheetRedirectFlags.Collectible)
                 && this.dataManager.GetExcelSheet<LSheets.Addon>(context.Language)
                        .TryGetRow(150, out var collectibleSymbol))
        {
        ...
        uint eColParamValue)
    {
        switch (resolvedSheetName)
        {
            ...
            case "Status" when this.dataManager.GetExcelSheet<LSheets.Status>(context.Language)
                                   .TryGetRow(eRowIdValue, out var statusRow):
    ...
    private bool TryResolveHeadAll(in SeStringContext context, in ReadOnlySePayloadSpan payload)
    {
        ...
        foreach (var p in str)
        {
            ...
            {
                context.Builder.Append(
                    Encoding.UTF8.GetString(p.Body.Span).ToUpper(true, true, false, context.Language));
                continue;
            }
    ...
    private bool TryResolveFixedMapLink(in SeStringContext context, ref ReadOnlySePayloadSpan.Enumerator enu)
    {
        ...
        if (this.dataManager.GetExcelSheet<LSheets.Addon>(context.Language).TryGetRow(rowId, out var addonRow))
        {
    ...
    private bool TryResolveFixedTimeRemaining(in SeStringContext context, ref ReadOnlySePayloadSpan.Enumerator enu)
    {
        ...
        {
            if (this.dataManager.GetExcelSheet<LSheets.Addon>(context.Language).TryGetRow(48, out var addonRow))
            {
                context.Builder.Append(addonRow.Text);
            }
        }
    ...
    private bool TryResolveFixedAutoTranslation(
        in SeStringContext context,
        in ReadOnlySePayloadSpan payload,
        int e0Val,
        int e1Val)
    {
        ...
        {
            context.Builder.Append(
                this.nounProcessor.ProcessNoun(
                    new NounParams()
                    {
    ...
    private unsafe bool TryGetGNumDefault(uint parameterIndex, out uint value)
    {
        ...
        ref var gp = ref this.macroDecoder.GetGlobalParameters();
        if (parameterIndex >= gp.MySize)
        {
            return false;
        }

        var p = this.macroDecoder.GetGlobalParameters()[parameterIndex];
        switch (p.Type)
        {
    ...
    private unsafe bool TryProduceGStrDefault(LuminaSeStringBuilder builder, ClientLanguage language, uint parameterIndex)
    {
        var rtm = RaptureTextModule.Instance();
        if (rtm is null)
        {
    ...
    private unsafe bool TryResolveUInt(
        in SeStringContext context,
        in ReadOnlySeExpressionSpan expression,
        out uint value)
    {
        ...
        if (expression.TryGetPlaceholderExpression(out var exprType))
        {
            ...
            switch ((ExpressionType)exprType)
            {
                ...
                case ExpressionType.Second:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_sec;
                    return true;
                case ExpressionType.Minute:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_min;
                    return true;
                case ExpressionType.Hour:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_hour;
                    return true;
                case ExpressionType.Day:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_mday;
                    return true;
                case ExpressionType.Weekday:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_wday + 1;
                    return true;
                case ExpressionType.Month:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_mon + 1;
                    return true;
                case ExpressionType.Year:
                    value = (uint)this.macroDecoder.GetMacroTime()->tm_year + 1900;
                    return true;
                default:
                    return false;
            }
    ...
    public string ServiceName => "SeString Evaluator";
}
