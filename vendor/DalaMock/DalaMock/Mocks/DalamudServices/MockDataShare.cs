...
public class MockDataShare
{
    private readonly ILogger<MockDataShare> logger;
    ...
    public MockDataShare(ILogger<MockDataShare> logger)
    {
        this.logger = logger;
    }
    ...
    public void RelinquishData(string tag, DataCachePluginId callingPluginId)
    {
        ...
        if (cache.Data is IDisposable disposable)
        {
            ...
            {
                disposable.Dispose();
                this.logger.LogTrace("[DataShare] Disposed [{Tag:l}] after it was removed from all shares.", tag);
            }
            catch (Exception e)
            {
                this.logger.LogTrace(e, "[DataShare] Failed to dispose [{Tag:l}] after it was removed from all shares.", tag);
            }
        }
        else
        {
            this.logger.LogTrace("[DataShare] Removed [{Tag:l}] from all shares.", tag);
        }
    }
    ...
    internal IEnumerable<(string Tag, DataCachePluginId CreatorPluginId, DataCachePluginId[] UserPluginIds)> GetAllShares()
    {
        lock (this.caches)
        {
            return this.caches.Select(kvp => (kvp.Key, kvp.Value.Value.CreatorPluginId, kvp.Value.Value.UserPluginIds.ToArray()));
        }
    }
}
