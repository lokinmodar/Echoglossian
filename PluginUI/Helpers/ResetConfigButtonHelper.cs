...
public static class ResetConfigButtonHelper
{
  ...
  /// <summary>
  /// Draws a button that resets the plugin config to default values, with a confirmation popup.
  /// </summary>
  ...
  /// <param name="buttonLabel">Optional label for the button (defaults to "Reset Settings to Default").</param>
  public static void Draw(Config config, Action saveCallback, string buttonLabel = "Reset Settings to Default")
  {
    ...
    if (showResetPopup)
    {
      ...
      if (ImGui.BeginPopupModal(popupId, ref open, ImGuiWindowFlags.AlwaysAutoResize))
      {
        ...
        {
          ResetSettings(config, saveCallback);
