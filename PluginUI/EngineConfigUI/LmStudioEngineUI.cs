...
{
  private static List<OpenAITextModel> staticModels = LmStudioTextModelDefaults.PredefinedModels;
  private static List<OpenAITextModel> liveModels = new();
  private static bool modelsFetched = false;
  ...
  public static bool Draw(Config config, PromptTemplateManager promptManager)
  {
    ...
    changed |= FieldValidationHelper.ValidatedInputText(Resources.APIEndpoint, ref config.LmStudioBaseUrl, 300, out isEndpointInvalid);
    ...
    if (config.UseLmStudioAuth)
    {
      ...
      changed |= FieldValidationHelper.ValidatedInputText(Resources.APIKey, ref config.LmStudioApiKey, 300, out isApiKeyInvalid);
    }

    ImGui.Checkbox(Resources.FetchLiveModels, ref config.UseLiveLmStudioModelList);
    ...
    if (config.UseLiveLmStudioModelList && !modelsFetched)
    {
      _ = Task.Run(async () =>
      {
        await LmStudioModelManager.RefreshAsync(config.LmStudioBaseUrl, config.UseLmStudioAuth ? config.LmStudioApiKey : null);
        modelsFetched = true;
      });
    }
    ...
    float temp = config.LmStudioTemperature;
    ...
    PromptEditorUI.Draw(promptManager, PromptType.LmStudio, PromptTemplateManager.DefaultPrompt, TransEngines.LmStudio.ToString());
