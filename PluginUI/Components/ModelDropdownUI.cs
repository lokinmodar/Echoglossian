...
public static class ModelDropdownUI
{
        ...
        out LlmTextModel? selectedModel)
    {
        ...
        string GetEngineTierGroup(LlmTextModel model)
        {
            var tier = model.Id switch
            {
                ...
                var id when id.StartsWith("gemini-2.5-flash") => Resources.ModelTierGemini15,
                var id when id.StartsWith("gemini-2.5-pro") => Resources.ModelTierGeminiPro,
    ...
    private static int BuildDefaultSort(LlmTextModel model)
    {
        return model.Id switch
        {
            ...
            var id when id.StartsWith("gemini-2.5-flash") &&
                !id.StartsWith("gemini-2.5-flash-lite") => 0,
            var id when id.StartsWith("gemini-2.5-flash-lite") => 1,
            var id when id.StartsWith("gemini-2.5-pro") => 2,
