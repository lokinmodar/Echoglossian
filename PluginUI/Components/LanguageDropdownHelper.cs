...
public static class LanguageDropdownHelper
{
  ...
  /// A dictionary where the key is the original index of the language, and the value is a <see cref="LanguageInfo"/> object containing details about the language.
  /// </param>
  public static void Initialize(Dictionary<int, LanguageInfo> languagesDictionary)
  {
    _cachedSortedLanguages = languagesDictionary
        .Select(kv => new LanguageEntry { OriginalIndex = kv.Key, Name = kv.Value.LanguageName })
        .OrderBy(entry => entry.Name)
        .ToList();

    _cachedDisplayNames = _cachedSortedLanguages.Select(l => l.Name).ToArray();
  }
  ...
  /// </summary>
  /// <param name="originalIndex">The original index of the language in the unsorted list.</param>
  ...
  public static int MapOriginalToSorted(int originalIndex)
  {
    ...
    int found = _cachedSortedLanguages.FindIndex(e => e.OriginalIndex == originalIndex);
  ...
  /// </summary>
  /// <param name="sortedIndex">The index of the language in the sorted dropdown list.</param>
  ...
  {
    if (_cachedSortedLanguages == null || sortedIndex < 0 || sortedIndex >= _cachedSortedLanguages.Count)
    {
      return 0;
    }
  ...
  /// <summary>
  /// Draws the language dropdown using ImGui, returning true if the value changed.
  /// </summary>
  /// <param name="selectedOriginalIndex">The original index of the selected language. This value will be updated if the selection changes.</param>
  ...
  public static bool DrawLanguageDropdown(ref int selectedOriginalIndex, string label = "Language")
  {
    ...
    int currentSortedIndex = MapOriginalToSorted(selectedOriginalIndex);
    bool changed = ImGui.Combo(label, ref currentSortedIndex, _cachedDisplayNames, _cachedDisplayNames.Length);
