...
public partial class Echoglossian
{
  ...
  private readonly HashSet<string> translatedBattleTalkTexts = new();

  private BattleTalkMessage lastBattleTalkMessage;
  ...
  private unsafe void ManageBattleTalk()
  {
    Task.Run(() =>
    {
      for (var i = 0; i < 5; i++)
      {
        ...
        {
          nameToTranslate = MemoryHelper.ReadSeStringAsString(
                  out _,
        ...
        if (this.translatedBattleTalkTexts.Contains(
                    Sanitizer.Sanitize(textToTranslate)))
        {
          Thread.Sleep(this.delayBetweenTriesToTranslateBattleTalk);
          continue;
        }
        ...
        PluginLog.Debug(
                $"ManageBattleTalk text to translate {nameToTranslate}: {textToTranslate}");

        this.lastBattleTalkMessage = new BattleTalkMessage(
                nameToTranslate,
                textToTranslate,
                originalSenderNameLang:
                ClientStateInterface.ClientLanguage.Humanize(),
                translatedBattleTalkMessage: string.Empty,
                originalBattleTalkMessageLang:
                ClientStateInterface.ClientLanguage.Humanize(),
        ...
        var foundBattleTalk =
                this.FindAndReturnBattleTalkMessage(
                    this.lastBattleTalkMessage);
        if (foundBattleTalk == null)
        {
          var textTranslation = this.Translate(textToTranslate);
          var nameTranslation = nameToTranslate.IsNullOrEmpty()
                  ? string.Empty
                  : this.Translate(nameToTranslate);
  ...
  private unsafe void TranslateBattleTalkReplacing()
  {
    ...
    try
    {
      ...
      if (this.configuration.TranslateNpcNames && nameNode != null &&
          !nameNode->NodeText.IsEmpty)
      {
        var translatedSName =
            this.lastBattleTalkMessage.TranslatedSenderName;
        ...
        if (this.configuration
            ...
            translatedSName != null)
        {
          translatedSName = this.RemoveDiacritics(
              translatedSName,
      ...
      var translatedBTMessage =
          this.lastBattleTalkMessage.TranslatedBattleTalkMessage;

      if (this.configuration
              .RemoveDiacriticsWhenUsingReplacementTalkBTalk &&
          translatedBTMessage != null)
      {
        translatedBTMessage = this.RemoveDiacritics(
            translatedBTMessage,
            this.SpecialCharsSupportedByGameFont);
      }
  ...
  private unsafe void TranslateBattleTalkUsingImGuiAndSwapping()
  {
    ...
    try
    {
      ...
      var nameToTranslate = MemoryHelper.ReadSeStringAsString(
          out _,
          (nint)nameNode->NodeText.StringPtr.Value);
      var textToTranslate = MemoryHelper.ReadSeStringAsString(
          out _,
      ...
      PluginLog.Debug(
          $"TranslateBattleTalkUsingImGuiAndSwapping text to translate {nameToTranslate}: {textToTranslate}");
      ...
      var textTranslation = this.Translate(textToTranslate);
      var nameTranslation = this.configuration.TranslateNpcNames
          ? nameToTranslate.IsNullOrEmpty()
              ? string.Empty
              : this.Translate(nameToTranslate)
          : nameToTranslate;
      ...
      {
        PluginLog.Debug(
                  "TranslateBattleTalkUsingImGuiAndSwapping SenderName task");
        var nameId = this.currentSenderTranslationId;
        var senderTranslation = nameToTranslate;
      ...
      {
        PluginLog.Debug(
                  "TranslateBattleTalkUsingImGuiAndSwapping BattleTalk task");
        var id = this.currentBattleTalkTranslationId;
        var translation = textToTranslate;
    ...
    {
      PluginLog.Debug(
          "TranslateBattleTalkUsingImGuiAndSwapping Exception: " + e);
    }
  }
  ...
  public unsafe void TranslateBattleTalkUsingImGuiWithoutSwapping()
  {
    ...
    try
    {
      ...
      PluginLog.Debug(
          $"TranslateBattleTalkUsingImGuiWithoutSwapping text to translate {nameToTranslate}: {textToTranslate}");

      Task.Run(() =>
      {
        PluginLog.Debug(
                  "TranslateBattleTalkUsingImGuiWithoutSwapping task");
        var textTranslation = this.lastBattleTalkMessage
        ...
        var nameId = this.currentSenderTranslationId;
        var senderTranslation = nameTranslation;
        this.senderTranslationSemaphore.Wait();
        if (nameId == this.currentSenderTranslationId)
        {
    ...
    {
      PluginLog.Debug(
          "TranslateBattleTalkUsingImGuiWithoutSwapping Exception: " + e);
    }
  }
      ...
      AddonArgs args)
  {
    ...
    {
      PluginLog.Debug(
          $"UiBattleTalkAsyncHandler: isInstanceContent {isInstanceContent}");
    }
    ...
    switch (type)
    {
      ...
      case AddonEvent.PreDraw:
        ...
        {
          this
              .TranslateBattleTalkUsingImGui(); // had disabled due to concurrency issues for now
        }
        else
        {
