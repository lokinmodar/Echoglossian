// <copyright file="ToDoPersistenceTests.cs" company="lokinmodar">
...
/// <summary>
///     Covers the dedicated ToDo persistence contract.
/// </summary>
public sealed class ToDoPersistenceTests
{
    ...
    /// </summary>
    [Fact]
    public void FindToDoText_ReusesExclusiveToDoRow()
    {
        ...
        try
        {
            using (var context = new EchoglossianDbContext(configDir))
            {
                ...
                context.Add(new ToDoText
                {
                    AddonName = "ToDo",
                    OriginalTextsAsText = JsonConvert.SerializeObject(new[] { "Halatali", "Clear the Hall of the Cesti: 0/1", "???" }),
                    OriginalLang = "en",
                    TranslatedTextsAsText = JsonConvert.SerializeObject(new[] { "Halatali", "Limpe o Hall of the Cesti: 0/1", "???" }),
                    TranslationLang = "pt-BR",
                    TranslationEngine = 0,
                    GameVersion = "test-version",
                    SourceContentHash = "todo-hash",
            ...
            Assert.NotNull(plugin.FindToDoText(new ToDoText
            {
                AddonName = "ToDo",
                OriginalTextsAsText = JsonConvert.SerializeObject(new[] { "Halatali", "Clear the Hall of the Cesti: 0/1", "???" }),
                OriginalLang = "en",
