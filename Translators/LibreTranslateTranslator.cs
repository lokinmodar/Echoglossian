...
public class LibreTranslateTranslator : ITranslator
{
  ...
  private static string DetermineEndpoint(Config config)
  {
    ...
    {
      LibreTranslateInstanceType.Com => "https://libretranslate.com/translate",
      LibreTranslateInstanceType.De => "https://libretranslate.de/translate",
      LibreTranslateInstanceType.Custom => config.LibreTranslateUrl?.TrimEnd('/') + "/translate",
  ...
  /// <summary>
  /// Asynchronously translates the specified text from the source language to the target language using LibreTranslate.
  /// </summary>
  ...
  public async Task<string> TranslateAsync(string text, string sourceLanguage, string targetLanguage)
  {
    try
    {
      ...
      this.pluginLog.Debug($"Sending LibreTranslate request to {this.endpoint} for: {fixedText}");
      ...
      {
        string clean = FixText(translated);
  ...
  public async Task<List<string>> GetSupportedLanguagesAsync()
  {
    ...
    try
    {
      string langUrl = this.endpoint.Replace("/translate", "/languages");
    ...
    {
      this.pluginLog.Warning($"Failed to fetch LibreTranslate languages: {ex.Message}");
      return new List<string> { "en", "es", "fr", "de", "it", "pt", "auto" };
    }
  }
}
