// <copyright file="GoogleTranslator.cs" company="lokinmodar">
// Copyright (c) lokinmodar. All rights reserved.
// Licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International Public License license.
// </copyright>

using Newtonsoft.Json;
using Newtonsoft.Json.Linq;

namespace Echoglossian.Translators
{
  /// <summary>
  /// Provides translation services using Google Translate APIs.
  /// </summary>
  public class GoogleTranslator : ITranslator
  {
    private readonly IPluginLog pluginLog;
    private readonly Config? config;

    /// <summary>
    /// Shared static HTTP client instance for sending requests.
    /// </summary>
    private static readonly HttpClient HttpClient = new();

    /// <summary>
    /// Defines the available Google Translate endpoints and required headers/query parameters.
    /// </summary>
    private readonly Dictionary<int, (string Url, Dictionary<string, string> Headers, Dictionary<string, string> QueryParams)> translateEndpoints =
        new()
        {
            {
                0,
                ("https://translate.google.com/m", new Dictionary<string, string>
                {
                    { "User-Agent", "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.6998.108 Mobile Safari/537.36" },
                },
                new Dictionary<string, string>())
            },
            {
                1,
                ("https://translate-pa.googleapis.com/v1/translateHtml", new Dictionary<string, string>
                {
                    { "User-Agent", "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.6998.108 Mobile Safari/537.36" },
                    { "X-Goog-API-Key", "AIzaSyATBXajvzQLTDHEQbcpq0Ihe0vWDHmO520" },
                },
                new Dictionary<string, string>())
            },
            {
                2,
                ("https://dictionaryextension-pa.googleapis.com/v1/dictionaryExtensionData", new Dictionary<string, string>
                {
                    { "x-referer", "chrome-extension://mgijmajocgfcbeboacabfgobmjgjcoja" },
                },
                new Dictionary<string, string>
                {
                    { "strategy", "2" },
                    { "key", "AIzaSyA6EEtrDCfBkHV8uU2lgGY-N383ZgAOo7Y" },
                })
            },
        };

#nullable enable

    /// <summary>
    /// Initializes a new instance of the <see cref="GoogleTranslator"/> class.
    /// </summary>
    /// <param name="pluginLog">Logger for plugin debugging and warnings.</param>
    /// <param name="config">Plugin configuration options.</param>
    public GoogleTranslator(IPluginLog pluginLog, Config? config)
    {
      this.pluginLog = pluginLog;
      this.config = config;
    }

    /// <inheritdoc />
    public string Translate(string text, string sourceLanguage, string targetLanguage)
    {
      this.pluginLog.Debug("inside GoogleTranslator translate method");

      try
      {
        return this.config?.GoogleTranslateVersion switch
        {
          1 => this.TranslateUsingV1(text, sourceLanguage, targetLanguage),
          2 => this.TranslateUsingV2(text, sourceLanguage, targetLanguage).GetAwaiter().GetResult() ?? string.Empty,
          _ => this.TranslateUsingV0(text, sourceLanguage, targetLanguage),
        };
      }
      catch (Exception e)
      {
        this.pluginLog.Error(e.ToString());
        throw;
      }
    }

    /// <inheritdoc />
    public async Task<string?> TranslateAsync(string text, string sourceLanguage, string targetLanguage)
    {
      this.pluginLog.Debug("inside GoogleTranslator translateAsync method");

      try
      {
        return this.config?.GoogleTranslateVersion switch
        {
          1 => await Task.FromResult(this.TranslateUsingV1(text, sourceLanguage, targetLanguage)),
          2 => await this.TranslateUsingV2(text, sourceLanguage, targetLanguage),
          _ => await this.TranslateUsingV0Async(text, sourceLanguage, targetLanguage),
        };
      }
      catch (Exception e)
      {
        this.pluginLog.Error(e.ToString());
        throw;
      }
    }

    private string TranslateUsingV0(string text, string sourceLanguage, string targetLanguage)
    {
      (var url, var headers, _) = this.translateEndpoints[0];
      url = $"{url}?hl=en&sl={sourceLanguage}&tl={targetLanguage}&q={Uri.EscapeDataString(Echoglossian.FixText(text))}";

      var request = new HttpRequestMessage(HttpMethod.Get, url);
      request.Headers.TryAddWithoutValidation("User-Agent", headers["User-Agent"]);

      var response = HttpClient.Send(request);
      using var reader = new StreamReader(response.Content.ReadAsStream());
      return Echoglossian.FormatStreamReader(reader.ReadToEnd());
    }

    private async Task<string?> TranslateUsingV0Async(string text, string sourceLanguage, string targetLanguage)
    {
      (var url, var headers, _) = this.translateEndpoints[0];
      url = $"{url}?hl=en&sl={sourceLanguage}&tl={targetLanguage}&q={Uri.EscapeDataString(Echoglossian.FixText(text))}";

      var request = new HttpRequestMessage(HttpMethod.Get, url);
      request.Headers.TryAddWithoutValidation("User-Agent", headers["User-Agent"]);

      var response = await HttpClient.SendAsync(request);
      using var reader = new StreamReader(await response.Content.ReadAsStreamAsync());
      return Echoglossian.FormatStreamReader(await reader.ReadToEndAsync());
    }

    private string TranslateUsingV1(string text, string sourceLanguage, string targetLanguage)
    {
      this.pluginLog.Debug("Google Translate V1 endpoint logic is not implemented.");

      // Example payload to send:
      /*
       * [
       *   [
       *     [
       *       "Assuming that Rishushu's associates at the Baert Trading Company are in possession of the same document, I suspect that the next attack on our supply caravan will strike right there─in the Central Shroud, due southwest from the White Wolf Gate."
       *     ],
       *     "auto",
       *     "pt-BR"
       *   ],
       *   "wt_lib"
       * ]
       */

      // Example JSON response:
      /*
       * [
       *   [
       *     "Supondo que os associados de Rishushu na Companhia Comercial Baert estejam de posse do mesmo documento, suspeito que o próximo ataque à nossa caravana de suprimentos ocorrerá bem ali, no Sudário Central, a sudoeste do Portão do Lobo Branco."
       *   ],
       *   [
       *     "en"
       *   ]
       * ]
       */

      throw new NotImplementedException("Google Translate V1 endpoint logic is not implemented.");
    }

    private async Task<string?> TranslateUsingV2(string text, string sourceLanguage, string targetLanguage)
    {
      (var url, var headers, var queryParams) = this.translateEndpoints[2];
      string parsedText = Echoglossian.FixText(text);
      string endpoint = $"{url}?language={targetLanguage}&key={queryParams["key"]}&term={Uri.EscapeDataString(parsedText)}&strategy={queryParams["strategy"]}";

      var request = new HttpRequestMessage(HttpMethod.Get, endpoint);
      foreach (var header in headers)
      {
        request.Headers.TryAddWithoutValidation(header.Key, header.Value);
      }

      var response = await HttpClient.SendAsync(request);
      var content = await response.Content.ReadAsStringAsync();

      // Example JSON response:
      /*
       * {
       *   "status": 200,
       *   "translateResponse": {
       *     "translateText": "Supondo que os associados de Rishushu na Baert Trading Company estejam de posse do mesmo documento, suspeito que o próximo ataque à nossa caravana de suprimentos ocorrerá ali mesmo - no Sudário Central, a sudoeste do Portão do Lobo Branco.",
       *     "detectedSourceLanguage": "en",
       *     "outputLanguage": "pt-BR",
       *     "sourceText": "Assuming that Rishushu's associates at the Baert Trading Company are in possession of the same document, I suspect that the next attack on our supply caravan will strike right there─in the Central Shroud, due southwest from the White Wolf Gate."
       *   }
       * }
       */

      try
      {
        JObject json = JObject.Parse(content);
        string? translatedText = json.SelectToken("translateResponse.translateText")?.ToString();

        if (!string.IsNullOrEmpty(translatedText))
        {
          this.pluginLog.Debug("Translated Text: " + translatedText);
          return translatedText;
        }

        this.pluginLog.Error("Translation not found in response.");
        return string.Empty;
      }
      catch (JsonException ex)
      {
        this.pluginLog.Error("Error parsing JSON: " + ex.Message);
        return string.Empty;
      }
    }
  }
}
